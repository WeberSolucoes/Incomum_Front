...
import { CidadeCreateRequest, DepartamentoCreateRequest, SituacaoTuristicoCreateRequest } from "../../utils/apiObjects";
import { apiCreateSituacaoTuristico, apiDeleteSituacaoTuristico,apiGetSituacaoTuristicoId, apiUpdateSituacaoTuristico } from "../../services/Api";
...
const SituacaoTuristico: React.FC = ({ onBackClick }) => {
    const { setCodigo,codigo } = useCodigo(); // Assumindo que useCodigo fornece o código da unidade
    const [request, setRequest] = useState<SituacaoTuristicoCreateRequest>({} as SituacaoTuristicoCreateRequest);
    ...
    const [stu_codigo, setVenCodigo] = useState<number | null>(null); // Inicialmente nulo ou 
    const [cpfValido, setCpfValido] = useState<boolean | null>(null);
    const [showModal, setShowModal] = useState(false);
    const activeTab = useSelector((state: RootState) => state.tabs.activeTab);

    useEffect(() => {
        if (!activeTab || activeTab !== 'Situação Turistico') {
        ...
        const fetchData = async () => {
            if (!codigo) return;
            try {
                const response = await apiGetSituacaoTuristicoId(codigo);
                const unidade = response.data;
                setRequest(unidade);
                setVenCodigo(unidade.stu_codigo); // Define o ID do vendedor
    ...
    const handleDeleteClick = () => {
        if (stu_codigo !== null && !showModal) { // Verifica se o modal não está aberto
    ...
    const handleConfirmDelete = async () => {
        if (stu_codigo !== null) {
            setLoading(true);
            try {
                await apiDeleteSituacaoTuristico(stu_codigo);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!request.stu_descricao) {
            toastError("O campo Situação é obrigatório.");
            setLoading(false);
            return;
        }
        ...
        try {
            let response;
            if (request.stu_codigo) {
                response = await apiUpdateSituacaoTuristico(request.stu_codigo, request);
            } else {
                const { stu_codigo, ...newRequest } = request;
                response = await apiCreateSituacaoTuristico(newRequest);
            }
    
            if (response.status === 200 || response.status === 201) {
                toastSucess("Situção Turistico salva com sucesso");
    
                // Atualize o `cid_codigo` no estado após criação bem-sucedida
                if (!request.stu_codigo && response.data && response.data.stu_codigo) {
                    setRequest(prev => ({
                        ...prev,
                        stu_codigo: response.data.stu_codigo
                    }));
                    setVenCodigo(response.data.stu_codigo); // Atualize também o estado `cid_codigo`
                }
            } else {
                toastError("Erro ao salvar a Situação Turistica");
            }
        } catch (error: any) {
    ...
    const handleReset = () => {
        setRequest({} as SituacaoTuristicoCreateRequest);
        setSelectedAreas([]);
        setRua('');
        setNumero('');
        setCidade('');
    ...
    return (
        <form className="erp-form" onSubmit={handleSubmit}>
            ...
            <div className="form-row">
                <div className="form-group">
                    <label htmlFor="stu_descricao">Situação Turistico</label>
                    <input
                        type="text"
                        id="stu_descricao"
                        name="stu_descricao"
                        value={request.stu_descricao || ''}
                        onChange={handleInputChange}
                        style={{width:'100%'}}
                    />
                </div>
            ...
            <div className="form-row">
                ...
                <button
                    style={{color:'white',backgroundColor:'#0152a1',marginLeft: request.stu_codigo ? '14px' : '0px',display: request.stu_codigo ? 'none' :''}}
                    type="button"
                    className="reset-btn"
                    onClick={handleReset}
                >
...
export default SituacaoTuristico;
