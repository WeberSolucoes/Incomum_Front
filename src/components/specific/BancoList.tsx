...
const BancoList: React.FC = ({ isActive, state }) => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Banco - ${truncatedDescricao}` 
            : 'Cadastro Banco'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetBanco();
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Banco</h1>
                ...
                <>
                    ...
                    <Banco onBackClick={handleBackClick} /> {/* Renderiza o componente de cadastro/edição */}
                </>
            )}
        </div>
    );
