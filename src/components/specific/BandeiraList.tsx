...
import { apiGetBandeira, apiGetMoeda, apiGetPais, apiGetUnidades } from '../../services/Api';
...
const BandeiraList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Bandeira - ${truncatedDescricao}` 
            : 'Cadastro Bandeira'; // Título padrão se não houver descrição
    };
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Bandeira</h1>
                ...
                <>
                    ...
                    <Bandeira onBackClick={handleBackClick} /> {/* Renderiza o componente de cadastro/edição */}
                </>
            )}
        </div>
    );
