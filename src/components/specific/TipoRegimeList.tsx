...
import { apiGetCep, apiGetCompanhia, apiGetMoeda, apiGetPais, apiGetRegime, apiGetUnidades } from '../../services/Api';
...
const TipoRegimeList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Regime - ${truncatedDescricao}` 
            : 'Cadastro Regime'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetRegime();
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.tre_codigo,
                descricao: item.tre_descricao,
            }));
    ...
    const columns = [
        ...
        { field: 'descricao', header: 'Tipo Regime' },
    ];
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Tipo Regime</h1>
