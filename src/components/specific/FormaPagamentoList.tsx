...
import { apiGetCidade, apiGetDepartamento, apiGetFormaPagamento, apiGetPais, apiGetUnidades } from '../../services/Api';
...
const FormaPagamentoList: React.FC = () => {
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetFormaPagamento();
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.for_codigo,
                descricao: item.for_descricao,
            }));
            ...
            toastError('Erro ao buscar a Forma de Pagamento');
        } finally {
    ...
    const columns = [
        ...
        { field: 'descricao', header: 'Forma Pagamento' },
    ];
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Forma De Pagamento</h1>
