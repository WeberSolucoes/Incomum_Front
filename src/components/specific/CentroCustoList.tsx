...
import { apiGetCentroCusto } from '../../services/Api';
...
const CentroCustoList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Centro De Custo - ${truncatedDescricao}` 
            : 'Cadastro Centro De Custo'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetCentroCusto();
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.cta_codigo,
                descricao: item.cta_descricao,
            }));
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Centro Custo</h1>
