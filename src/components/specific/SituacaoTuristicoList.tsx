...
import { apiGetCidade, apiGetDepartamento, apiGetPais, apiGetSituacaoTuristico, apiGetUnidades } from '../../services/Api';
import { toastError } from '../../utils/customToast';
import { Button } from 'primereact/button';
import { InputText } from 'primereact/inputtext';
import { useCodigo } from '../../contexts/CodigoProvider'; // Importa o contexto
import SituacaoTuristico from './SituacaoTuristico';
import useEnterKey from '../../hooks/useEnterKey';

const SituacaoTuristicoList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Situação Turistico - ${truncatedDescricao}` 
            : 'Cadastro Situação Turistico'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetSituacaoTuristico();
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.stu_codigo,
                descricao: item.stu_descricao,
            }));
    ...
    const columns = [
        ...
        { field: 'descricao', header: 'Situação Turistico' },
    ];
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Situação Turistico</h1>
                    ...
                    <GenericTable 
                        filteredItems={items} 
                        emptyMessage="Nenhuma Situação Turistico encontrada" 
                ...
                <>
                    ...
                    <SituacaoTuristico onBackClick={handleBackClick} /> {/* Renderiza o componente de cadastro/edição */}
                </>
            )}
        </div>
    );
...
export default SituacaoTuristicoList;
