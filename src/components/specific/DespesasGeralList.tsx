...
import { apiGetCidade, apiGetDepartamento, apiGetDespesasGeral, apiGetPais, apiGetUnidades } from '../../services/Api';
...
const DespesasGeralList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Despesas Geral - ${truncatedDescricao}` 
            : 'Cadastro Despesas Geral'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.grc_codigo,
                descricao: item.grc_descricao,
            }));
            ...
            toastError('Erro ao buscar Despesas Geral');
        } finally {
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Despesas Geral</h1>
...
export default DespesasGeralList;
