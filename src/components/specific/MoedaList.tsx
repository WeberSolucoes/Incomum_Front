...
import { apiGetMoeda, apiGetPais, apiGetUnidades } from '../../services/Api';
...
const MoedaList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Moeda - ${truncatedDescricao}` 
            : 'Cadastro Moeda'; // Título padrão se não houver descrição
    };   
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetMoeda();
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.moe_codigo,
                descricao: item.moe_descricao,
            }));
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Moedas</h1>
