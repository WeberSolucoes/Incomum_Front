...
const DespesasList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Despesas - ${truncatedDescricao}` 
            : 'Cadastro Despesas'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const mappedData: UnidadesListResponse[] = response.data.map((item: any) => ({
                codigo: item.mgr_codigo,
                descricao: item.mgr_descricao,
            }));
    ...
    const columns = [
        ...
        { field: 'descricao', header: 'Despesas' },
    ];
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Despesas</h1>
                    ...
                    <GenericTable 
                        filteredItems={items} 
                        emptyMessage="Nenhuma Despesas encontrado" 
