...
import { apiGetCep, apiGetMoeda, apiGetPais, apiGetUnidades } from '../../services/Api';
...
const CepList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Cep - ${truncatedDescricao}` 
            : 'Cadastro Cep'; // Título padrão se não houver descrição
    };
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await apiGetCep();
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Cep</h1>
                ...
                <>
                    ...
                    <Cep onBackClick={handleBackClick} /> {/* Renderiza o componente de cadastro/edição */}
                </>
            )}
        </div>
    );
