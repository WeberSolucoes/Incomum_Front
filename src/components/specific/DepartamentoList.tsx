...
import { apiGetCidade, apiGetDepartamento, apiGetPais, apiGetUnidades } from '../../services/Api';
...
const DepartamentoList: React.FC = () => {
    ...
    const getTitle = () => {
        ...
        return truncatedDescricao 
            ? `Cadastro Departamento - ${truncatedDescricao}` 
            : 'Cadastro Departamento'; // Título padrão se não houver descrição
    };
    ...
    const columns = [
        ...
        { field: 'descricao', header: 'Departamento' },
    ];
    ...
    return (
        <div>
            {view === 'list' ? (
                <>
                    <h1 style={{color:'#0152a1'}}>Consulta de Departamento</h1>
